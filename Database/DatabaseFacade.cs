...
{
    public enum FileLabel:byte { NOTHING = 0x01, RED = 0x02, GREEN = 0x03, BLUE = 0x04, ORANGE = 0x05, PURPLE = 0x06};

    public static class DatabaseFacade
    {
        private static IDatabase database;

        static DatabaseFacade() => database = new InMemoryBase();
        /// <summary>
        ...
        /// <returns></returns>
        public static FileLabel GetFileLabel(string Path) => database.GetFileLabel(Path);
        /// <summary>
        ...
        public static void SetFileLabel(string Path, FileLabel Label) => database.SetFileLabel(Path, Label);
        ...
        public static void SetFilesLabelAndSaveBase(string[] Paths, FileLabel Label)
        {
            ...
            {
                if (File.GetAttributes(Paths[i]) != FileAttributes.System)
        ...
        /// <param name="Paths">Полные пути к каталогам или файлам для которых необходимо сохранить метку</param>
        /// <param name="Label">Метка файлов</param>
        public static async void SetFilesLabelAndSaveAsync(string[] Paths, FileLabel Label) => await Task.Run(() => SetFilesLabelAndSaveBase(Paths, Label));
        /// <summary>
        /// Асинхронный метод сохранения базы данных
        /// </summary>
        public static void SaveBaseAsync() => database.SaveBaseAsync();
